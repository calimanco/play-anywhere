...
You can use the npx command to run without installation. (npm 5.2+)

```bash
npx play-anywhere [path] [options]
```

After the server is started, you can access http://localhost:3000.
...
- `webpackConfig` Please refer to Webpack [document](https://webpack.docschina.org/configuration/) 。
- `serveStaticConfig` Please refer to serve-static [document](http://expressjs.com/en/resources/middleware/serve-static.html) 。
...
It is mapped to `http://localhost:3000/public` by default. You can change it through `staticPath` in the configuration.  
More customization can be done through `serveStaticConfig` in the configuration.  
...
You can use `public/other.js` in `third/index.ejs` now:
...
Use the following method to obtain the type declaration.
...
#### Q：When a static file conflicts with a file generated by Webpack, which one takes precedence?

A：The files generated by Webpack will take precedence.
